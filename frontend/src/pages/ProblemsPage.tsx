...
export default function ProblemsPage() {
  ...
  const fetchProblems = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Não foi possível carregar os problemas. Tente novamente mais tarde."
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="bg-card rounded-xl shadow-elegant border border-border/50 overflow-hidden p-8">
            <div className="text-center">
              ...
              <div className="sm:space-x-4 flex flex-col sm:flex-row gap-2 sm:gap-0 items-center justify-center">
                <button
                  onClick={() => navigate("/")}
                  className="px-6 py-3 bg-button hover:bg-buttonhover text-foreground rounded-lg font-medium transition-all"
                >
                  Voltar ao início
                </button>
              </div>
